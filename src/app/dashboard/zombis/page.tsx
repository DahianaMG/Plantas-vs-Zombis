import { ZombiCard } from '@/components/zombi-card';
...
export default async function ZombisPage() {
  ...
  if (user.error) {
    ...
    return (
      ...
      <div className='grid grid-cols-3 pt-2 pb-4 mt-2'>
        <div></div>
        <div className="flex justify-center text-4xl text-lime font-bold text-center" style={{textShadow: "2px 2px 2px darkgreen"}}>Zombis</div>
      ...
      <div className="items-center justify-center flex flex-col gap-4">
            <div className="grid grid-cols-2 grid-flow-row sm:grid-cols-5 md:grid-cols-5 lg:grid-cols-5 flex items-center justify-center gap-y-8 gap-x-2 flex-wrap p-3">
              {data?.map((zombi: any) => (
                <ZombiCard zombi={zombi} key={zombi.id} />
              ))}
            </div>
            ...
            <a href="/dashboard/zombis/create" className='cursor-portal'>
             <p className="text-lime font-bold" style={{textShadow: "2px 2px 2px darkgreen"}}>Crear zombi</p>
            </a>
            </button>
        </div>
      </div>
