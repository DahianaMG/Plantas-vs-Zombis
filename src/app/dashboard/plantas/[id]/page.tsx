...
export default async function PlantaPorIdPage({ params }: any) {
  ...
  return (
    ...
    <div className="flex flex-col gap-4 w-full justify-center items-center">
      <div className='grid grid-cols-3 pt-2 pb-4 w-full'>
        ...
        <div className='flex items-center justify-center'>
          <button
            ...
            <a href="/dashboard/plantas" className="cursor-portal text-lime font-bold" style={{textShadow: "2px 2px 2px darkgreen"}}>Atrás</a>
          </button>
        </div>
      ...
      <h3><span className="text-lime font-bold" style={{textShadow: "2px 2px 2px darkgreen"}}>Tipo:</span> {data?.tipo}</h3>
      <h3><span className="text-lime font-bold" style={{textShadow: "2px 2px 2px darkgreen"}}>Efectividad:</span> {data?.efectividad}</h3>
      <h3><span className="text-lime font-bold" style={{textShadow: "2px 2px 2px darkgreen"}}>Precio:</span> {data?.precio}</h3>
      </div>
      ...
      <button
        ...
        <a href={`/dashboard/plantas/${data?.id}/edit`} className="cursor-portal text-lime font-bold" style={{textShadow: "2px 2px 2px darkgreen"}}>Editar</a>
      </button>
      <DeletePlantButton planta={data} />
      </div>
